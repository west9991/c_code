...
//<<parameter setting>>//
	...
	for(int nstep=0; nstep<=nend; nstep++){
		if(nstep%INI==0){
			char inip_delete[256];
			sprintf(inip_delete, "initial_profile_change_%04d.dat", nstep);
			FILE *finip_delete = fopen(inip_delete, "w");
		}
	}
	char del_th[256];
	sprintf(del_th, "vel_th_", "vel_th_beta%.2f_dx%.2f_nn%d.dat", beta, dx*1.0E6, (int)nn);
