...
int main(void)
{
    ...
    fclose(delete_vel);
    FILE *delete_vol = fopen("vol.data", "w");
    fclose(delete_vol);
    ...
    fprintf(fini, "Interface energy    [J/m^2]      =  %16.7e\n", gamma);
    fprintf(fini, "Interface mobility  [m^4/(Js)]   =  %16.7e\n", amobi);
    ...
    for(int nstep=1; nstep<=nend; nstep++){
        ...
        if(nstep%nvel==0){
            ...
            fprintf(fp_v,"%6d %16.7e %16.7e %6d %16.7e\n", nstep, (float)nstep*dt, x05*1.0E6, i, u05*1.0E6);
        ...
        if(nstep%nout==0){
            ...
            fprintf(fp_v, "%6d %16.7e %16.7e\n", nstep, (float)nstep*dt, vol*1.0E12);
            fclose(fp_v);
        }
